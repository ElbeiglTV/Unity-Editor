...
namespace Old
{
    ...
    public class DecisionTreeEditor : EditorWindow
    {
        ...
        [MenuItem("Window/Decision Tree Editor")] // permite tener el boton en la pesta�a window para abrir la ventana editable
        ...
        void Butons() // botones extraidos de la funcion onGUI()
        {
            ...
            if (rootNode == null)
            {
                ...
                if (GUILayout.Button("Create New Decision Tree")) // muestra el boton y el boton devuelve un bool dependiendo de si lo tocaste o no
                {
                    ...
                    AddToTypeQueue(rootNode, NodeType.nonConectedNode); // a�ade el nodo raiz a la cola
                    AddToDicionaryAddQueue(rootNode, new Rect(50, 50, 200, 100)); // Posici�n inicial del nodo ra�z
                }
            }
            else
            {
        ...
        void RestartDecicionTree()
        {
            ...
            nodeRects.Clear();
            AddToTypeQueue(rootNode, NodeType.nonConectedNode);
            AddToDicionaryAddQueue(rootNode, new Rect(mousePos.x, mousePos.y, 200, 100)); // Posici�n inicial del nodo ra�z
        }
        ...
        private Texture2D MakeTex(int width, int height, float cornerRadius, Color color)
        {
            ...
            for (int y = 0; y < height; y++)
            {
                for (int x = 0; x < width; x++)
                {
                    ...
                    // Si est� dentro del radio, pinta el p�xel con el color; de lo contrario, pinta transparente
        ...
        {
            // Calcula la longitud y el �ngulo de la l�nea
            float length = Vector2.Distance(start, end);
            float angle = Mathf.Atan2(end.y - start.y, end.x - start.x) * Mathf.Rad2Deg;

            // Dibuja una textura (l�nea) entre los puntos de inicio y fin
        ...
        void DrawNode(DecisionNode node)
        {
                ...
                if (node.trueNode != null) // si el nodo verdadero no esta vasio
                {
                    ...
                    if (GUILayout.Button("Restart Node", GUILayout.ExpandWidth(false))) // se dibuja el boton de resetear nodo
                    {
                        ...
                        node.trueNodeID = node.trueNode.UniqueID; // se le asigna la id del nuevo nodo a la id del true node
                                                                  //se a�ade a las queue correspondientes-
            ...
            GUILayout.EndArea();
            // Dibujar l�neas de conexi�n aqu�
            Handles.color = Color.green;
        ...
        GUIStyle ButonStyle()
        {
            ...
            buttonStyle.fontSize = 8; // Tama�o del texto dentro del bot�n
            buttonStyle.fixedHeight = 15; // Altura fija del bot�n
            return buttonStyle;
        }
        GUIStyle EnumStyle()
        {
            GUIStyle enumPopupStyle = new GUIStyle();
            enumPopupStyle.fontSize = 8; // Tama�o del texto dentro del EnumPopup
